...
6. Start Codex and Claude deep reviews of the committed Ready candidate in parallel and wait for both. The coordinator narrows only from a matching pair of completed records; a standalone review, partial pair, mismatched profile, or same-HEAD target always leaves the coordinated gate eligible for a full requested range. Pass `--base` to choose the base explicitly.
   ...
   Fix what is broken. A finding earns a fix when it names an input or state where the current code produces a wrong result, loses data, or violates a stated contract — the reviewers supply that as the failure scenario. A finding that only makes working code better is a deferral, whatever severity it carries. Repeat until no finding names a reproducible wrong behavior; findings will keep arriving after that, and acting on them is how a change stops converging.

   Stop repairing an area after three rounds. An area is the behavior a finding names; treat two findings as one area when they name the same code path or the same user-visible behavior, and record that grouping in the dispositions so the count is visible. When the third consecutive round in one area returns only findings that the context already dispositioned, findings that reverse the previous round's accepted fix, or findings without a new failure scenario, the area is not converging: record the remaining items in the deferred list, note the stop in the context file's dispositions, and continue to Ready. A re-raised disposition is not a new finding, and a finding that undoes the previous round's fix is judged against that round's disposition unless it brings a failure scenario that disposition did not consider. Reducing or removing the contested behavior is a valid outcome of the stop when it keeps the acceptance criteria intact.
...
`review:codex` checks a clean committed checkout and verifies that `HEAD` and the worktree do not change. Its implementation phase is a standalone intermediate review unless launched by `review:implementation`; it reads the requested base-to-HEAD change and never updates coordinated final history. It uses native `codex review --base` in a read-only sandbox. Its standalone spec phase reads the fixed Artifact Share version and unresolved comments, then passes them to `codex exec` in a read-only sandbox. When launched by `review:spec`, it reads the coordinator's immutable local snapshot instead.

`review:claude` is a thin launcher with a 30-minute limit. Its implementation phase is also a standalone intermediate review and uses the same effort for the session and `/code-review <effort> <base-sha>...<head-sha>`. `--level` remains a compatibility alias for `--effort`; a supplied alias is used as the effort, and conflicting explicit values fail. Its standalone spec phase reads the current Artifact Share version and unresolved comments; the supplied version id prevents review of a stale revision. When launched by `review:spec`, it reads the same immutable local snapshot as Codex.
...
For an implementation gate, provide the current context file to the coordinator:

```sh
pnpm review:implementation -- --context-file <path>
pnpm review:implementation -- --base <ref> --context-file <path>
# From the fourth coordinated round on a branch, after applying the stop rule:
pnpm review:implementation -- --context-file <path> --acknowledge-round-cap
```

The context file is the coordinator's record for the reviewers: purpose, current acceptance criteria, scope notes, and a `## Dispositions` section that lists every prior finding with its outcome, as `fixed` (with the commit), `deferred` (with the deferred-list entry), `non-actionable` (with the reason), `follow-up` (with the issue), or `stop` (the stop rule was applied to an area); `None yet` on the first round. The coordinator refuses a context without that section, a context whose Dispositions items do not start with one of those outcomes, and a context that contains the prompt delimiters. It also counts the distinct earlier heads reviewed on the branch (a rerun of the current head is not a new round): from the fourth round it stops with `ROUND_CAP` until you rerun with `--acknowledge-round-cap`, a transient flag whose meaning is that the stop rule above was applied and the remaining items are in the deferred list (the branch count is the mechanical proxy for the per-area rule). Every `Dispositions` section is checked, and one with no items must say `None yet`. Both reviewers receive it verbatim and are instructed not to re-raise a dispositioned finding without a new failure scenario. It reads the nonempty context once, copies it to one immutable temporary file outside the repository, and starts both commands concurrently on the same clean commit. Both reviewer launch functions receive the same resolved base SHA, expected HEAD SHA, role profile, review-only contract, current acceptance criteria, prior finding dispositions, and a live process-local capability for the coordinator's activity lock. They start the provider CLIs asynchronously without an inherited lock-bypass environment variable. The coordinator waits for both to finish, verifies the target and clean worktree before delivery and before final history, and prints only their complete final results. Pair history narrows only when both prior reviewer records match the same head and final profile; an explicit `--base` always wins, and a same-HEAD run rereads the full requested change. A standalone review or partial pair cannot narrow or satisfy this gate. The coordinator bounds failure diagnostics and removes its temporary context file after completion. The implementation and spec gates, standalone `review:claude`, `review:codex`, and `review:cursor`, `screens:capture`, `walkthroughs:capture`, and `critique:tasks` share one activity lock per worktree (a file under the shared Git directory, held by a child process that exits with its parent). A second command started in the same worktree is refused at once, including one started by a reviewer descendant. A capture or critique writes under the worktree and restarts services while a gate expects the tree it snapshotted to stay untouched; the lock makes the order explicit instead of surfacing as "worktree changed during review".
...
If a required local tool (`codex`, `claude`, `gh`, the platform's `lockf`/`flock` utility, or the Artifact Share CLI used for spec readback) is unavailable, stop before any remote write and report the missing dependency.
