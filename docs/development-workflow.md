...
2. If a specification is required, start Codex and Claude deep reviews of its fixed final version in parallel, wait for both, classify every finding together, and repeat both reviews in parallel on each new version until no blocker remains. `review:spec` ends the gate itself after three rounds and reports `ROUND_CAP`; carry anything still open as a deferral at step 9 rather than opening a fourth round.
...
6. Start Codex and Claude deep reviews of the committed Ready candidate in parallel and wait for both. The first round reads the whole change; every later round automatically reads only what changed since the last completed review, because its question is whether the fixes broke something rather than whether the change is perfect. Pass `--base` to force a full re-read.
...
`review:codex` checks a clean committed checkout and verifies that `HEAD` and the worktree do not change. Its implementation phase runs native `codex review`, against `origin/main` on the first round and against that reviewer's own last reviewed head afterwards. Pass `--base` only to force a full re-read; an explicit base disables the narrowing. Its standalone spec phase reads the fixed Artifact Share version and unresolved comments, then passes them to `codex exec` in a read-only sandbox. When launched by `review:spec`, it reads the coordinator's immutable local snapshot instead.

`review:claude` is a thin launcher with a 30-minute limit. Its implementation phase runs Claude Code's built-in `/code-review high` against `origin/main...HEAD` on the first round, and against its own last reviewed head afterwards. Pass `--base` only to force a full re-read. Its standalone spec phase reads the current Artifact Share version and unresolved comments; the supplied version id prevents review of a stale revision. When launched by `review:spec`, it reads the same immutable local snapshot as Codex. The default `high` review is the final gate. Use `--level low` only for a quick intermediate pass, never as gate evidence.
...
The coordinator stores its bounded state under `artifactshare/spec-review` in the repository's shared Git common directory. Main and linked-worktree checkouts therefore use one state and lock namespace. A SHA-256 of the canonical Artifact id selects the state and lock names; the Artifact URL itself is never used as a filename, and equivalent URL spellings cannot split the review history. The state contains only generation, revision, original baseline metrics, at most three reviewed version/fingerprint entries, and the latest finding ids, reviewers, and severities. It does not contain the specification body, reviewer prose, credentials, tokens, an attempt log, or a permanent receipt. Updates use a same-directory temporary file and atomic rename, so a failed reviewer or interrupted process leaves the last completed state intact. The per-spec OS file lock rejects a second coordinator before duplicate reviews start and is released by the operating system if the holder process exits; lock-file contents and stale PID cleanup are not part of correctness.

On the first run after this storage change, the coordinator recognizes an existing trusted review-state comment, reads its referenced record when needed, and writes the bounded equivalent locally. It never deletes or edits the legacy record or comment. Once local state exists, ordinary review has exactly two Artifact Share accesses: the initial snapshot read and the final unchanged-input readback. The coordinator prints the baseline metrics needed for the next correction. For correction reviews, pass an untracked temporary `--dispositions-file`. It contains `baseline_metrics`, a `prior_findings` array exactly covering the persisted Codex and Claude ids (including an explicitly empty array), and a same-length `dispositions` array keyed by those ids. Each disposition is `fixed`, `follow_up`, `non_actionable`, or `rewrite`; set `repeated` or `contradiction` to `true` when applicable. The coordinator refuses incomplete classification. It also ends the gate itself after three rounds, reporting `ROUND_CAP` with the findings still open so the change proceeds and carries them as deferrals; it does not wait for anyone. Starting a new generation with `--reset` still requires owner approval. After an approved full rewrite, start a new local generation with `--reset`. Do not commit the temporary dispositions file.
