...
The coordinator stores its bounded state under the repository's Git-private path (`git rev-parse --git-path artifactshare/spec-review`). A SHA-256 of the Artifact URL selects the state and lock names; the URL itself is never used as a filename. The state contains only generation, revision, original baseline metrics, at most three reviewed version/fingerprint entries, and the latest finding ids, reviewers, and severities. It does not contain the specification body, reviewer prose, credentials, tokens, an attempt log, or a permanent receipt. Updates use a same-directory temporary file and atomic rename, so a failed reviewer or interrupted process leaves the last completed state intact. The per-spec directory lock rejects a second coordinator before duplicate reviews start and recovers a lock whose owner process no longer exists.
