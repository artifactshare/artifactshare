...
The Codex/Claude deep-review pair is required only when a change affects schema, authorization, billing, delivery, or the public/private boundary. For those changes, review the exact Artifact Share version or commit that will be handed to implementation or Ready, let both independent reviews finish, and classify every finding before changing the target. Ordinary code, workflow, and normative documentation changes still receive an appropriate self-review and targeted validation, but do not require this pair. A change to this workflow policy itself must receive the Codex/Claude deep-review pair before landing.

When a specification is required, apply that same proportional rule to its fixed final version. A boundary-sensitive change uses the dual deep-review pair before implementation; an ordinary change uses self-review and targeted validation. Implementation starts only after the applicable review and validation are complete.
...
When the dual-review pair is required, the gate passes when neither reviewer has an unresolved blocker on the reviewed target, not when both reviewers report zero findings. A quick `low` review or a single-reviewer pass may help during development but never replaces that final pair. For ordinary changes, the self-review and targeted validation are the applicable gate. A specification may have its initial review and at most two correction reviews. A fourth review is refused and requires a coherent rewrite from the original scope lock and acceptance criteria.
...
Changing the specification after its gate invalidates that gate. Changing the implementation after its gate invalidates that gate. Finish mechanical corrections before the final review; if the version or commit changes afterward, repeat the applicable review against the new fixed target. When the dual pair is required, start both deep reviews again in parallel and wait for both results so one correction pass can address the complete finding set. Keep finding dispositions in the normal task or reviewer session, and summarize the final review, any follow-ups, and any deferred findings in the pull request or task record. Do not create review receipts, digests, attempt logs, or review-specific push guards.
...
2. If a specification is required, apply the proportional review rule to its fixed final version. For a boundary-sensitive change, start Codex and Claude deep reviews in parallel, wait for both, classify every finding together, and repeat both reviews in parallel on each new version until no blocker remains. For an ordinary change, complete the self-review and targeted validation. `review:spec` returns nonzero after three rounds with `ROUND_CAP` and an unreviewed target; rewrite the specification from the original scope lock before continuing.
...
6. For a boundary-sensitive change, initialize the branch's immutable objective scope, then start Codex and Claude deep reviews of the committed Ready candidate in parallel and wait for both. A blocker must name one of the scope's failures, a supported scenario within its trusted inputs, and the wrong result. For an ordinary change, complete the appropriate self-review and targeted validation. Classify broader requests as follow-ups or non-actionable.

   Review the exact committed target that will be handed to Ready. If a correction changes that target, repeat the applicable review against the new commit before continuing. The workflow does not impose a fixed number of correction commits; continue until the reviewed target has no unresolved blocker and the selected validation is complete. When the dual pair is required, the coordinator narrows the reviewed range only from a matching completed pair; pass `--base` to choose the base explicitly.
...
9. Push the final commit normally and run `pnpm pr:ready` with `--task-usage-report <path>` when the PR includes a workflow-usage block. The report, when supplied, must be a sanitized report produced by the maintainer task-usage operation for this checkout's `HEAD`; it contains only public-safe rows, totals, coverage, the target commit SHA, and the generated markdown block. `pr:ready` validates the report's coverage and arithmetic, requires a matching reason for every row-level unknown, verifies the report target against local and remote `HEAD`, and validates that the generated block is the one in the PR body. If no workflow-usage block is present, no report is required. It rechecks the PR state immediately before the Ready mutation and after it; if the body, head, or Draft state changes during that interval, it attempts to revert Ready and fails closed. It verifies the Draft targets `main` and required PR checks have succeeded before calling `gh pr ready`. When the diff contains UI changes, it stops until you confirm that affected states and registered tasks were captured at desktop and mobile on the reviewed commit, the walkthrough evidence, standalone captures, and relevant source were critiqued, and no UI changed afterward; after those checks, rerun it with `pnpm pr:ready -- --ui-gate-complete`. Before Ready, classify every deferred review finding honestly in the PR or task record; use `--deferred <text>` (or `--deferred-file <path>`) for each finding, or `--no-deferred` when there are none.
...
After the PR lands, no separate post-landing disposition record is required. Keep any deferred review finding honestly classified in the PR or task record; a deferral is not a claim that the finding was fixed.
...
The orchestrator owns coordination and adjudication. For initial implementation, it chooses between two approved profiles by scope judgment: routine, local, and pattern-following changes use Luna at max; changes involving concurrency, state, migration, recovery, or integrity across multiple conditions use Sol at medium. This choice is not a classifier and does not authorize unilateral escalation. Reviewers receive repair work only after the parent adopts a concrete finding. For a boundary-sensitive change, the final gate remains an independent Codex/Claude pair on one fixed target; ordinary changes use self-review and targeted validation.
...
For a boundary-sensitive implementation gate, create a strict JSON scope no larger than 4 KiB with the immutable objective, named failures, trusted inputs, and manual recovery details. Do not encode a fixed number of correction commits in the workflow scope. Initialize it once on an attached task branch and inspect the persisted scope after a session change or compaction:
...
Before Ready, a small, safe workflow-usage block may be included in the PR. If present, update it after every execution, retry, repair, failed or interrupted run, and re-review. One row is one execution (implementation, one Codex review, one Claude review, or similar); a paired review is two rows, and conversational topic phases are not separate rows. The maintainer task-usage operation keeps the detailed start/end record and emits the sanitized block; the canonical `pr:ready` validator validates it when present. If the block is omitted, no usage report is required. The public block records requested and reported model/effort metadata, elapsed duration, normalized input, cache-read/cache-write detail, output, total tokens, and result coverage. It intentionally omits timestamps, local paths, session IDs, and private data. Keep this canonical report schema and `pr:ready` validator as the only machinery for this record; do not add separate scripts, collectors, receipts, or CI checks. Normalized input is raw input plus cache-read and cache-write input for both providers. Cache read/write values are subsets of normalized input. Provider-reported reasoning is included in output; do not add reasoning or other nested token fields again. Total is normalized input plus output. Sum token categories and total. Summed run durations add every row's elapsed duration, including parallel runs, and are not wall-clock elapsed time. Calculate wall span from the earliest included start to latest included end, including gaps. Mark missing boundaries or usage `partial` or `unknown`, never zero. Publish only safe summaries.
...
- For a change in the dual-review scope, start the Codex and Claude reviews concurrently through the gate (standalone reviews in one worktree run one at a time under the activity lock), and do not run commands that can change HEAD or the worktree until both finish. Ordinary changes use self-review and targeted validation.
- A boundary-sensitive specification gate applies to one exact Artifact Share version. Any new version requires a new pair of deep reviews before implementation; ordinary specifications are re-reviewed against their new fixed version by the owner.
- A boundary-sensitive implementation gate applies to one exact commit. Any later commit requires a new pair of deep reviews before Ready; ordinary changes require a new self-review and targeted validation for the changed target.
- Up to three open PRs may be active concurrently in this repository; each remains subject to its own branch, fixed-target, validation, and public/private boundary checks.
- A Draft PR is the review workspace; commit and push fixes normally.
- `pr:ready` requires a clean worktree, a Draft for the current branch targeting `main`, a pushed local `HEAD`, and successful required checks. A workflow-usage report is optional; when present, it must target that `HEAD` and its generated block must be the one in the PR body.
