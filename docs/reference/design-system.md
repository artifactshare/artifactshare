...
画面キャプチャとタスク walkthrough を入力に、批評 agent が UI を評価するときの観点の正本。観点の追加・変更はこの章の更新で行う。画面台帳は画面責務、タスク台帳は利用者・状況・目的・遷移・完了自己確認、walkthrough は時系列証拠、批評は評価を所有し、同じ情報を別の台帳へ複製しない。

導線の発見可能性と一覧の制限値は、[発見経路と一覧の制限値を分けて設計する](../decisions/discovery-and-list-limit-roles.md)の問いも照合する。

### 画面観点

上位基準は「北極星 (投稿→反応→再投稿の輪) に照らして、この画面は担当の段 (画面台帳の `loop`) を前へ進めているか」。6 観点はその内訳である。visual 層は高精度モデルで PNG と関連ソースを読み、画面台帳の `role` / `primaryAction` / `loop` が破綻する指摘だけを blocker 候補にする。

1. **役割の単一性**: 画面の役割が 1 つに絞れているか。画面台帳 (`scripts/screen-ledger.mjs`) の `role` / `primaryAction` と実際の画面が一致しているか。
2. **語彙**: 表示文言が用語集 ([glossary](./glossary.md)) の表示語に従っているか。lint の `copy-glossary/no-violations` rule が deny list で拾えない言い回しの揺れも見る。
...
task 層は walkthrough と証拠を読み、次の 8 観点をタスクごとに評価する。agent-mediated のタスクでは、代理操作を実行する agent ではなく、その報告から結果を確認する owner を利用者として扱う。

1. **利用者・persona と mediation**: その場の利用者、前提知識、本人操作か代理操作かが実際の利用状況と合うか。
2. **目的**: 画面単体の操作完了ではなく、タスク台帳の `goal` へ進めるか。
3. **状態**: `start`、`action`、`pending`、`success`、`failure`、`recovery`、`next` の各状態が区別できるか。
4. **手がかり**: 次に何を見て何を選ぶかを判断できる情報があるか。
5. **feedback**: 操作を受け付けたか、成功したか、失敗したかを本人が判断できるか。
6. **制約**: 権限、公開範囲、形式、保存先など、結果を左右する条件を行動前に把握できるか。
7. **回復**: 失敗理由を理解し、入力や対象を失わずにやり直せるか。
8. **習熟**: 初回利用者が理解でき、慣れた利用者が不要な往復なく素早く完了できるか。

指摘は「利用者はこの時点で X を判断する必要がある。そのため Y が存在する／不足している」という因果で記述する。色や余白などの表面的な描写だけをタスク指摘にしない。タスク台帳の `confirmation` を本人による完了自己確認の基準として必ず照合する。
タスク台帳の `acceptedBehavior` は、owner が採用した現在の製品判断として照合する。矛盾、再現した故障、または利用者への害を示す新しい証拠があれば再検討する。同じ代替案を新しい証拠なしに再提案しない。

### 入力と triage

批評入力には、対象画面の PNG、影響タスクの desktop/mobile walkthrough、タスクと persona、関連ソース、walkthrough が記録した通知、frame の URL と load 状態、failed request、clipboard、該当する場合は CLI 結果を含める。入力が欠ける場合、批評 agent はブラウザを起動したり推測したりせず `NEEDS INPUT` を返す。
...
- **measure-first**：製品上の問題である可能性はあるが、頻度、主因、利用者への影響のいずれかが不明で、対処によって製品の複雑さが増える所見。
  ...
  分子と分母を実際に観測できる方法と、判定条件の根拠を示す。根拠のない閾値や実行できない計測案を作らない。
  判定時期までは対処 UI を追加しない。
...
2. **コピー用語検査 (`copy-glossary/no-violations`)**: 禁止語、表示語の置換、語彙の組み合わせなど、表示文言を静的に判定できる指摘。許容表現と禁止表現の根拠は [glossary](./glossary.md) に置く。
