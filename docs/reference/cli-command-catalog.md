...
| `login` | 実装済 | 明示的に認証だけを行う。`--preset agent` は投稿・更新をブラウザ承認時に選んだ1プロジェクトへ制限する（閲覧は workspace 公開の成果物と、承認者へ共有された非公開プロジェクトに及ぶ）。`--json` では承認 URL 発行後に OS ブラウザ起動を試み、成否は stderr の pending イベント `browser_open` で返す | なし | `--profile` / `--preset agent\|unrestricted` / `--allow-plaintext-token-store` |
