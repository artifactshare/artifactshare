...
- Under the default thresholds, the service applies a publication limit to Free workspaces less than 14 days old and refuses another publication when it observes 20 counted files in the rolling 24-hour window. Each file counts once based on its most recent change to link visibility, or its creation with link visibility while it is still shared by link. Because the check and publication are separate operations, concurrent requests can pass together and 20 is not an absolute ceiling. Another publication can succeed when the observed count drops below the limit; existing links remain available. Reaching the limit can start a `publish_burst` review, but enforcement does not depend on the review starting or succeeding, and its result does not lift the limit. Workspace link policies start with a 30-day default and no maximum expiry, and can set finite defaults and maximums from 1 to 365 days or allow no expiration.
- A human operator may pause or resume link sharing after a review or report. Pausing blocks anonymous URL-only access while the owner and explicitly granted viewers retain access; the owner receives the reason by email and can appeal from the file page. Artifact Share stores the submitted appeal text with the file's event history and forwards an abbreviated copy to operators in an alert. Resuming sends another owner notice. The automated review never pauses, hides, or throttles a link by itself.
