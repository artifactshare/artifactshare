...
export function authRequiredError(baseUrl: string): CliError {
  ...
  return cliError({
    ...
    hint: `For an attended local agent, run ${CLI_INVOCATION} login --profile default --preset agent. In CI, issue a token at ${url} and inject ${TOKEN_ENV_VAR} or ${TOKEN_OPTION}. Shared agent platforms use a workspace-managed bot outside the model sandbox.`,
    agentRecoverable: false,
    requiresHuman: true,
    recovery: { kind: 'ask_human' },
...
export function profileReauthRequiredError(
  ...
  return cliError({
    ...
    hint: `For an attended local agent, run ${loginCommand} --preset agent. In CI, issue a token at ${tokensUrl(baseUrl)} and inject ${TOKEN_ENV_VAR} or ${TOKEN_OPTION}. Shared agent platforms use a workspace-managed bot outside the model sandbox.`,
    agentRecoverable: false,
    requiresHuman: true,
    recovery: { kind: 'ask_human' },
    details: {
