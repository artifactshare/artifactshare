...
export function authRequiredError(baseUrl: string): CliError {
  ...
  return cliError({
    ...
    hint: `With a user present, run ${CLI_INVOCATION} login --profile default; for an attended local agent, add --preset agent. In CI, issue a token at ${url} and inject ${TOKEN_ENV_VAR} or ${TOKEN_OPTION}. Shared agent platforms use a workspace-managed bot outside the model sandbox.`,
    agentRecoverable: false,
    requiresHuman: true,
    recovery: { kind: 'ask_human' },
...
export function profileReauthRequiredError(
  ...
  return cliError({
    ...
    hint: `With a user present, run ${loginCommand}; the saved authorization preset is reused. In CI, issue a token at ${tokensUrl(baseUrl)} and inject ${TOKEN_ENV_VAR} or ${TOKEN_OPTION}. Shared agent platforms use a workspace-managed bot outside the model sandbox.`,
    agentRecoverable: false,
    requiresHuman: true,
    recovery: { kind: 'ask_human' },
    details: {
