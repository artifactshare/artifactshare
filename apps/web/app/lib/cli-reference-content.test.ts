...
test('describes the deletion alias in both locales', () => {
  expect(
    cliReferenceContent('en').commands.find(
      (command) => command.path === 'artifacts delete',
    )?.role,
  ).toMatch(/Alias of delete.*permanently delete/)
  expect(
    cliReferenceContent('ja').commands.find(
      (command) => command.path === 'artifacts delete',
    )?.role,
  ).toMatch(/delete の別名.*完全に削除/)
})
