...
export const llmsTxt = [
  ...
  '- "Log out of Artifact Share here" — revokes a device-login credential before removing it locally.',
  ...
  'The CLI uses --link-expires-at <RFC3339 UTC> or --no-link-expiry; do not pass both. Free and Plus can use link sharing; Team workspace policy can disable it.',
  '',
  '## Notes',
  '',
...
export const capabilitiesMd = [
  ...
  `| logout | Revoke and remove the saved credential for a CLI profile. |`,
  ...
  'If a link operation fails, inspect the structured error recovery: `link_sharing_plan_required` is a legacy code (no plan limit applies today), `link_sharing_disabled` needs the workspace owner or a Team admin to enable the workspace setting, and `link_expiry_invalid` needs a future timestamp within policy or an allowed no-expiration request. Under the default thresholds, the MCP code `link-publish-rate-limited` and CLI code `link_publish_rate_limited` mean the service observed 20 counted files before a publication in a Free workspace less than 14 days old during its rolling 24-hour window; leave the file with specific people and follow the returned recovery instead of retrying unchanged input.',
