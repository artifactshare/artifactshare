  ...
  test('keeps the blocked and paused copy exact', () => {
    const expected = {
      en: [
        ...
        'Ask your IT administrator to allow HTTPS connections to artifactshare.com, *.sandbox.artifactshare.com, and *.artifactshare.link. The latter two domains securely isolate and display file content.',
        'Display paused',
      ...
      ja: [
        ...
        '社内ネットワークで利用する場合',
        'IT管理者へ、artifactshare.com、*.sandbox.artifactshare.com、*.artifactshare.linkへのHTTPS通信の許可を依頼してください。後者2つは、ファイルの内容を隔離して表示するための配信用ドメインです。',
        '表示を一時停止しています',
