...
type ExecuteBridgeRequestResult = BridgePublishResult

export async function executeBridgePublishIntent(input: {
  db: Kysely<DB>
  ...
  const { db, authority, user, idempotencyKey, metadata, files, origin, now } =
    input
  const contextResult = parseTrustedBridgeContext(metadata, authority, now)
  if (
    contextResult.kind !== 'ok' ||
    contextResult.context.requestId !== idempotencyKey
  ) {
  ...
  const bound = await bindTrustedBridgeRequest(db, authority, context)
  ...
  const ownedFiles = await verifyMultipartFiles(intent, files)
  if (ownedFiles.kind !== 'ok') return ownedFiles

  const digest = await computeTextSha256(
    JSON.stringify({
      operation: intent.operation,
      requestedAudience: intent.requestedAudience,
  ...
  if (lease.kind === 'completed') {
    ...
    if (
      intent.requestedAudience === 'private' &&
  ...
  if (intent.operation === 'set_visibility') {
  ...
  } else if (intent.contentKind === 'static_site') {
    execution =
      intent.operation === 'publish' || intent.operation === 'update'
  ...
  } else if (intent.contentKind !== 'file' || ownedFiles.files.length !== 1) {
    execution = { kind: 'forbidden-target' }
  } else if (intent.operation === 'publish') {
