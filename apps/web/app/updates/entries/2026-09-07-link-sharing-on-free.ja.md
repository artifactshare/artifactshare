...
Free のワークスペースでも、ファイルごとにリンク共有を選べるようになりました。共有リンクを知っている人は、Artifact Share にログインせずにファイルを閲覧できます。終了日の設定は Plus と同じで、ワークスペースの既定は 30 日です。ワークスペースのポリシーが許可していれば無期限にもできます。
