...
WHEN NEW.visibility = 'link'
  AND EXISTS (SELECT 1 FROM link_publication_attempts WHERE workspace_id = NEW.workspace_id AND shareable_id = NEW.id)
BEGIN
  INSERT INTO link_publications (workspace_id, shareable_id, latest_published_at)
  ...
  WHERE attempt.workspace_id = NEW.workspace_id
    ...
    AND (attempt.limit_applies = 0 OR (
  ...
  ON CONFLICT (workspace_id, shareable_id) DO UPDATE SET
    latest_published_at = MAX(link_publications.latest_published_at, excluded.latest_published_at);
  SELECT RAISE(ABORT, 'link publication quota exceeded') WHERE changes() = 0;
  UPDATE link_publication_attempts SET consumed = 1
  WHERE workspace_id = NEW.workspace_id AND shareable_id = NEW.id;
END;

CREATE TRIGGER link_publication_consume_transition_attempt
AFTER UPDATE OF visibility ON shareables
WHEN OLD.visibility <> 'link' AND NEW.visibility = 'link'
  AND EXISTS (SELECT 1 FROM link_publication_attempts WHERE workspace_id = NEW.workspace_id AND shareable_id = NEW.id)
BEGIN
  INSERT INTO link_publications (workspace_id, shareable_id, latest_published_at)
  SELECT workspace_id, shareable_id, published_at
...
AFTER INSERT ON shareables
WHEN NEW.visibility = 'link'
  AND NOT EXISTS (SELECT 1 FROM link_publication_attempts WHERE workspace_id = NEW.workspace_id AND shareable_id = NEW.id)
BEGIN
  INSERT INTO link_publications (workspace_id, shareable_id, latest_published_at)
...
BEGIN
  INSERT INTO link_publications (workspace_id, shareable_id, latest_published_at)
  VALUES (NEW.workspace_id, NEW.id, NEW.updated_at)
  ON CONFLICT (workspace_id, shareable_id) DO UPDATE SET
    latest_published_at = MAX(link_publications.latest_published_at, excluded.latest_published_at);
END;
